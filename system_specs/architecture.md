...
C4Container
    ...
    Boundary(c1, "Thermal Grace System") {
        Container(dashboard_ui, "Dashboard UI", "Python, Streamlit", "User interface for metrics, multi-user comfort, and LLM advice.")
        Container(dashboard_services, "Dashboard Services", "Python Threads", "Background workers for MQTT ingestion and weather polling.")
        Container(feedback_app, "User Feedback App", "Python, Streamlit", "Standalone survey app for capturing occupant context.")
    ...
    Rel(occupant, dashboard_ui, "Views personalized insights")

    Rel(feedback_app, filesystem, "Writes user context")
    Rel(dashboard_ui, filesystem, "Reads context / UI State")
    Rel(dashboard_services, filesystem, "Appends sensor logs")

    Rel(sensor_nodes, dashboard_services, "Streams telemetry", "MQTT")
    Rel(dashboard_services, dashboard_ui, "Updates shared state")
    Rel(dashboard_services, buienradar, "Fetches weather", "HTTPS")
    Rel(dashboard_ui, github_llm, "Generates recommendations", "HTTPS")
```
...
C4Component
    ...
    Container_Boundary(dashboard_boundary, "Dashboard Application") {
        ...
        Boundary(services_boundary, "Background Services") {
        ...
        Boundary(shared_boundary, "Shared Logic & State") {
